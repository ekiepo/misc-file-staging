...
function initialRegistry(request) {
  return {
    ...
    settings: {
      ...
      nextSequence: highestSequence(seedRecords) + 1,
    },
...
function normalizeRegistry(registry) {
  registry.deletedRecords = Array.isArray(registry.deletedRecords) ? registry.deletedRecords : [];
  const knownRecords = [...registry.records, ...registry.deletedRecords];
  const nextSequence = Math.max(Number(registry.settings.nextSequence || 0), highestSequence(knownRecords) + 1);
  registry.settings.nextSequence = nextSequence;
  return registry;
}
...
async function loadRegistry(request) {
  ...
  if (stored) return normalizeRegistry(JSON.parse(await streamToText(stored.stream)));
...
function validateAnalyticsRange(range) {
  if (range.from && !isIsoDate(range.from)) return false;
  if (range.to && !isIsoDate(range.to)) return false;
  return !(range.from && range.to && range.from > range.to);
}

async function scanSummary(range = {}) {
  const blobs = await listEvery(SCAN_PREFIX);
  const summary = new Map();
  for (const blob of blobs) {
    const parts = blob.pathname.split('/');
    const id = parts[2];
    const scanDate = parts[3];
    const visitorHash = parts[4]?.split('-')[0] || '';
    if (!ID_PATTERN.test(id)) continue;
    if (!isIsoDate(scanDate)) continue;
    if (range.from && scanDate < range.from) continue;
    if (range.to && scanDate > range.to) continue;
...
async function recordsWithAnalytics(registry, range = {}) {
  const summary = await scanSummary(range);
...
function nextId(registry) {
  normalizeRegistry(registry);
  const sequence = registry.settings.nextSequence;
  registry.settings.nextSequence += 1;
  return `${registry.settings.idPrefix}${String(sequence).padStart(3, '0')}`;
}
